...
- [Build the real token screen, chat UI, and streaming endpoint](issues/15-task-build-chat-ui.md): `/` is the app — server-side session check renders the token screen or chat, `POST /api/chat` streams OpenRouter's SSE behind the ticket-10 gate with ticket 08's prompt and ticket 09's `max_tokens`; the throwaway prototype and the create-next-app landing page are gone; **the index metadata had no chunk text** so `text` was added and the live index re-embedded — without that the model had nothing to ground on.
