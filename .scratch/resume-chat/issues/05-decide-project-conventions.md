...
**Amendment 2026-09-21** (from *Decide deployment mechanism, secrets, and environments*, human's call): secrets management changed from **varlock to plain Cloudflare dashboard / `wrangler secret put` Worker secrets** — varlock was eliminated from the repo. The tool choice above was made in good faith but the human judged it unnecessary for a 3-secret app; custody intent (server-side only, nothing in the repo) is unchanged. Local dev uses a gitignored `.env.local`.
